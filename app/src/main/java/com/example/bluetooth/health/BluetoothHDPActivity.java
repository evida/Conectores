...
public class BluetoothHDPActivity extends Activity {
	...
	private Handler mIncomingHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case BluetoothHDPService.RECEIVED_SYS:
				...
				String weightStr = "" + sys/100 +"."+sys%100;
    ...
    private void sendData() {
        ...
        //values
        String measurement_date = sdf.format(date);
        String measurement_time = stf.format(date);

        weightOb = new JSONObject();

        try {
            weightOb.put(Constants.getParameterDate(), measurement_date);
            weightOb.put(Constants.getParameterTime(), measurement_time);
            weightOb.put(Constants.getParameterSource(), "sampleWS");
            weightOb.put(Constants.getParameterWeight(), weight);
    ...
    private class RestConnection extends AsyncTask<Void, Void, String> {
        ...
        public RestConnection() {
            super();
            this.access_token = Constants.getAccessToken();
        }

        @Override
        protected void onPreExecute() {
            String dialogMessage = "A enviar dados para o servidor";
            progressDialog = ProgressDialog.show(BluetoothHDPActivity.this, "", dialogMessage, true, true);
        }

        @Override
        protected String doInBackground(Void... arg0) {
            String st = weightOb.toString();
            String response = "";
            ...
            try {
                String url = Constants.getPHRhost() + "" + Constants.getEntityWeight() + "?access_token=" + access_token;
                response = RestClient.executeHttpOauthPost(url, BuildPost.buildPost(weightOb));
            } catch (Exception e) {
                e.printStackTrace();
                Log.e(TAG, "Post error:BloodPressure");
                Log.i(TAG, "BloodPressure" + response);
            }
